...
function Row(growths, setValueCowId) {
    ...
    const setValue = (ccowId, gWeight) => {
        setValueCowId.setCowId(ccowId);
        setValueCowId.setGstatus('01');
        setValueCowId.closeInput();
        if (gWeight) setValueCowId.setInputStatus('edit')
        else setValueCowId.setInputStatus('add')
    }
    const row = growths.map((growth) => ({
        ...
        action: (<MonitorWeightIcon sx={{ cursor: "pointer", fontWeight: "bold" }} fontSize="small" color={null === growth.gWeight ? "disabled" : "success"}
            onClick={event => setValue(growth.ccowId, growth.gWeight)}
        />),
    }))
    return row;
}
